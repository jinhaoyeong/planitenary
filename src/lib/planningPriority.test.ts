...
const priorities = (itinerary: Itinerary): Record<string, string> => Object.fromEntries(
  buildPlanningMaterial('trip-1', itinerary).places.map((place) => [place.name, place.priority]),
);

describe('discovery priorities reach the planner', () => {
  it('carries a Must do from the discovery panel into planning material', () => {
    ...
    expect(priorities(trip)).toEqual({ 'Glico Man Sign': 'must-do' });
  });

  it('carries an Interested decision through the same save path', () => {
    const trip = savedTrip([kuromon, mint], { [kuromon.id]: 'interested', [mint.id]: 'interested' });

    expect(priorities(trip)).toEqual({
      'Kuromon Ichiba Market': 'interested',
      'Mint Museum': 'interested',
    });
  });

  it('resolves the whole shortlist the way the traveller marked it', () => {
    const trip = savedTrip([glico, kuromon, mint], {
      [glico.id]: 'must-do',
      [kuromon.id]: 'interested',
      [mint.id]: 'interested',
    });
    const material = buildPlanningMaterial('trip-1', trip);
  ...
  it('keeps the provider identity that the decision is resolved from', () => {
    const trip = savedTrip([glico], { [glico.id]: 'must-do' });
  ...
  it('resolves a legacy decision keyed by the provider place ID alone', () => {
  ...
  it('lets the current decision win over a stale bare-ID one for the same place', () => {
    const trip = savedTrip([glico], { 'osm-n3507545614': 'must-do', n3507545614: 'interested' });

    expect(priorities(trip)).toEqual({ 'Glico Man Sign': 'must-do' });
  });

  it('lets a provider-qualified decision win over a stale bare-ID one', () => {
    const trip = savedTrip([glico], { 'osm-n3507545614': 'must-do', n3507545614: 'interested' });
    ...
    expect(priorities(withoutCandidateId)).toEqual({ 'Glico Man Sign': 'must-do' });
  });

  it('does not let two providers sharing a raw place ID inherit each other’s decision', () => {
  ...
  it('keeps the canonical decision exact even when the raw ID is ambiguous', () => {
    const otherProvider = osmCandidate('n3507545614', 'Namba Parks', [34.6614, 135.5021], {
      id: 'google-n3507545614',
      provider: 'google',
    });
  ...
  it('does not let an ambiguous bare Must do constraint promote another provider’s place', () => {
    const otherProvider = osmCandidate('n3507545614', 'Namba Parks', [34.6614, 135.5021], {
      id: 'google-n3507545614',
      provider: 'google',
    });
  ...
  it('still honours a bare Must do constraint where the raw ID names one place', () => {
  ...
  it('resolves a decision for a place saved without its candidate ID', () => {
    const trip = savedTrip([glico], { [glico.id]: 'must-do' });
    ...
    expect(priorities(legacy)).toEqual({ 'Glico Man Sign': 'must-do' });
  });

  it('does not give one place another place’s decision because the names match', () => {
  ...
  it('reads the Must do constraint through the same canonical identity', () => {
    const trip = savedTrip([glico, kuromon], {}, {
  ...
  it('never promotes a skipped place, and never drops it silently either', () => {
  ...
  it('keeps a Must do ahead of optional places when the material is truncated', () => {
  ...
  it('changes the planning revision when the traveller changes a decision', () => {
    const interested = savedTrip([glico], { [glico.id]: 'interested' });
    const mustDo = savedTrip([glico], { [glico.id]: 'must-do' });

    expect(buildPlanningMaterial('trip-1', interested).revision)
      .not.toBe(buildPlanningMaterial('trip-1', mustDo).revision);
  });
});
